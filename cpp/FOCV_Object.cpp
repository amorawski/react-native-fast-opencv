...
jsi::Object FOCV_Object::convertToJSI(jsi::Runtime& runtime, const jsi::Value* arguments, size_t count) {
    ...
    switch(hashString(objectType.c_str(), objectType.size())) {
        ...
        case hashString("keypoint", 8): {
            auto keypoint = *FOCV_Storage::get<cv::KeyPoint>(id);

            jsi::Object pt(runtime);
            pt.setProperty(runtime, "x", jsi::Value(keypoint.pt.x));
            pt.setProperty(runtime, "y", jsi::Value(keypoint.pt.y));

            value.setProperty(runtime, "pt", pt);
            value.setProperty(runtime, "angle", jsi::Value(keypoint.angle));
            value.setProperty(runtime, "response", jsi::Value(keypoint.response));
            value.setProperty(runtime, "size", jsi::Value(keypoint.size));
            value.setProperty(runtime, "octave", jsi::Value(keypoint.octave));
            value.setProperty(runtime, "class_id", jsi::Value(keypoint.class_id));
        } break;
        case hashString("keypoint_vector", 15): {
            auto points = *FOCV_Storage::get<std::vector<cv::KeyPoint>>(id);

            auto array = jsi::Array(runtime, points.size());

            for (size_t i = 0; i < points.size(); i++) {
                const auto keypoint = points.at(i);
                ...
                item.setProperty(runtime, "pt", pt);
                item.setProperty(runtime, "angle", jsi::Value(keypoint.angle));
                item.setProperty(runtime, "response", jsi::Value(keypoint.response));
                item.setProperty(runtime, "size", jsi::Value(keypoint.size));
                item.setProperty(runtime, "octave", jsi::Value(keypoint.octave));
                item.setProperty(runtime, "class_id", jsi::Value(keypoint.class_id));
                array.setValueAtIndex(runtime, i, item);
            }
...
jsi::Object FOCV_Object::copyObjectFromVector(jsi::Runtime& runtime, const jsi::Value* arguments, size_t count) {
    ...
    switch(hashString(objectType.c_str(), objectType.size())) {
        ...
        case hashString("keypoint_vector", 15): {
            auto array = *FOCV_Storage::get<std::vector<cv::KeyPoint>>(vectorId);
            auto keypoint = array.at(index);
            createdId = FOCV_Storage::save(keypoint);
            return FOCV_JsiObject::wrap(runtime, "keypoint", createdId);
        } break;
    }
...
void FOCV_Object::addObjectToVector(jsi::Runtime& runtime, const jsi::Value* arguments, size_t count) {
    ...
    switch(hashString(objectType.c_str(), objectType.size())) {
        ...
        case hashString("keypoint_vector", 15): {
            auto &array = *FOCV_Storage::get<std::vector<cv::KeyPoint>>(vectorId);
            auto &object = *FOCV_Storage::get<cv::KeyPoint>(objectId);
            array.push_back(object);
        } break;
    }
}
